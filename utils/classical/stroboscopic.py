...
class PhasePortrait:
	...
	def __init__(self,nperiod,ny0,timepropagator,xmax=np.pi,pmax=np.pi,random=False,rotated=False,theta1=0,theta2=0,X0=0,Y0=0):
		...
		self.np0=int(pmax/(xmax+pmax)*ny0)
		...
		a=np.concatenate((self.x0,np.zeros(self.np0)))
		b=np.concatenate((np.zeros(self.nx0),self.p0))
		...
		if self.random==True:
			a=rd.randint(0,101,ny0)/100.0*2*self.xmax-self.xmax
			b=rd.randint(0,101,ny0)/100.0*2*self.pmax-self.pmax
			self.y0[:,0]=a
			self.y0[:,1]=b
