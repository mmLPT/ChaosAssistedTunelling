...
for i in range(1,ncell2+1):
	...
	wf1.setState("coherent",x0=xi,xratio=xratio)
	plt.plot(grid.x,np.abs(wf1.x))
	
	wf=wf+wf1
	xi=x0-i*2*np.pi
	wf2=WaveFunction(grid)
	wf2.setState("coherent",x0=xi,xratio=xratio)
	wf=wf+wf2
...
# ~ ax=plt.gca()
# ~ ax.plot(np.fft.fftshift(grid.p),np.fft.fftshift(np.abs(wf.p)**2),c="blue")
# ~ ax.plot(np.roll(np.flipud(np.fft.fftshift(grid.p)),1),np.fft.fftshift(np.abs(wf.p)**2),c="red")
# ~ plt.show()

np.savez("tempdata/wfqm",p=np.fft.fftshift(grid.p),wfp=np.fft.fftshift(np.abs(wf.p)**2))
...
np.savez("tempdata/N13-EPSC1-hm3d398",xL=pL,xR=pR,time=time)
