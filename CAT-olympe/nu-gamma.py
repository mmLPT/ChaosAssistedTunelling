import sys
sys.path.insert(0, '..')
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.colors as colors
import os

from utils.toolsbox import *
from utils.quantum import *
from utils.classical import *
from utils.plot.latex import *
from utils.systems.modulatedpendulum import *



# This scripts makes possibles to 
# 1. compute in // the free propagation for different quasi-momentum
# 2. gather and average the results
# 3. plot the averaged data
 
mode=sys.argv[1]
wdir=sys.argv[2]

if mode=="initialize":
	os.mkdir(wdir)
	os.mkdir(wdir+"trajectories")
	os.mkdir(wdir+"pictures")

if mode=="compute":
	# Loading input file
	inputfile=sys.argv[3]
	data=np.load(inputfile+".npz")
	
	# Description of the run
	description=data['description']

	# General physical parameters 
	N=int(data['N'])
	e=data['e']
	x0=data['x0']
	h=data['h']

	# Free propagation
	iperiod=int(data['iperiod']) #number of period

	# heff values
	gammamin=data['gammamin']
	gammamax=data['gammamax']

	data.close() # close the input file
	
	ngamma=int(sys.argv[4]) # number of runs for a given h
	runid=int(sys.argv[5])-1 # Id of the current run

	if runid==0: # This generate and parameters files in the working directory with read input (to avoid surprises)		
		np.savez(wdir+"params","w", description=description,N=N, e=e,h=h,x0=x0,gammamin=gammamin,gammamax=gammamax,ngamma=ngamma,iperiod=iperiod)

	# Create array to store "Left" and "Right" observables
	xR=np.zeros(iperiod)
	xL=np.zeros(iperiod)

	# Initialization of the grid for given h value
	gamma=np.linspace(gammamin,gammamax,ngamma)[runid]
	pot=PotentialMP(e,gamma)
	grid=Grid(N,h)
	
	# Create the Floquet operator
	fo=CATFloquetOperator(grid,pot)

	# Create the initial state: a coherent state localized in x0 with width = 2.0 in x
	wf=WaveFunction(grid)
	wf.setState("coherent",x0=x0,xratio=2.0)

	# Propagate the wavefunction over iperiod storing the observable every time
	for i in range(0,iperiod):
		xL[i]=wf.getxL()
		xR[i]=wf.getxR()
		fo.propagate(wf)

	A=max(xR[0],xL[0])
	xL=xL/A
	xR=xR/A

	np.savez(wdir+"trajectories/"+strint(runid),"w", xL = xL, xR=xR)


	time=2.0*np.linspace(0.0,1.0*iperiod,num=iperiod,endpoint=False)
	ax=plt.gca()
	ax.set_xlim(0,200)
	ax.set_ylim(0,1.0)
	s,nu,x0exp=convert2exp(gamma,h,x0)
	ax.set_title(r"$\varepsilon={:.2f} \quad \gamma={:.3f} \quad h={:.3f} \quad x0={:.1f}$".format(e,gamma,h,x0)+"\n"+r"$\varepsilon={:.2f} \quad s={:.3f} \quad \nu={:.3f} kHz \quad x_0={:.1f}^o$".format(e,s,nu/10**3,x0exp))
	plt.plot(time,xL, c="red")
	plt.plot(time,xR, c="blue")
	plt.savefig(wdir+"pictures/"+strint(runid)+".png") 

if mode=="final":
	data=np.load(wdir+"params.npz")
	ngamma=data['ngamma']
	iperiod=data['iperiod']
	gammamin=data['gammamin']
	gammamax=data['gammamax']
	e=data['e']
	h=data['h']
	x0=data['x0']
	data.close()

	iTF=int(iperiod)
	
	
	gammam=np.linspace(gammamin,gammamax,ngamma)
	nu=np.fft.rfftfreq(iTF,d=2.0)

	X,Y=np.meshgrid(gammam,nu)
	
	Z=np.zeros((int(iTF/2)+1,ngamma))
	time=2.0*np.linspace(0.0,1.0*iperiod,num=iperiod,endpoint=False)
	

	for ih in range(0,ngamma):
		data=np.load(wdir+"trajectories/"+strint(ih)+".npz")
		xL=data['xL']*np.exp(-30*time/time[iperiod-1])
		xR=data['xR']*np.exp(-30*time/time[iperiod-1])
		xLf=np.abs(np.fft.rfft(xL))
		xRf=np.abs(np.fft.rfft(xR))
		xLf[0]=0.0
		xRf[0]=0.0
		

		Z[:,ih]=(xLf+xRf)*0.5

	np.savez(wdir+"nu-gamma","w", gamma=X,nu=Y,tf=Z,e=e,h=h,x0=x0,iperiod=iperiod)
	ax=plt.gca()

	ax.set_xlabel("gamma")
	ax.set_ylabel("frequence")
	ax.set_title(r"$\varepsilon={:.3f} \quad h={:.3f}\quad x_0={:.3f}$".format(e,h,x0))
	ax.set_yscale("log")
	ax.set_ylim(1.0/iperiod,max(nu))

	cmap = plt.get_cmap('Greys')
	
	
	
	plt.pcolormesh(X,Y,Z,cmap=cmap)
	plt.savefig(wdir+"nu-gamma.png",bbox_inches = 'tight',format="png") 

	
	


