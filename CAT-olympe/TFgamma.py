...
if mode=="compute":
	...
	np.savez(wdir+str(indexgamma)+"/"+str(indexbeta),"w", beta=beta, xL = xL, xR=xR,xExp=xExp)
...
if mode=="process":
	...
	np.savez(wdir+str(igamma)+"/averaged","w",  xL = xLav, xR=xRav,xExp=xExpav,time=time)
...
if mode=="final":
	...
	plt.pcolormesh(gamma,omegas,density, norm=norm,cmap=cmap)
